...
# 7 is an odd integer and the maximum odd integer that occurs before 7 is 5. But 5 is not greater than the current integer 7. So 7 is printed.
...
odd = [i if i%2 else -1 for i in arr]
even = [i if i%2==0 else -1 for i in arr]
for i in range(n):
    if arr[i]%2==0:
        print(max(even[i:]),end=' ')
    else:
        print(max(odd[:i+1]),end=' ')