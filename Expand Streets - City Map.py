# Expand Streets - City Map
# The program must accept a matrix of size R*C representing the map of a city and an integer K as the input. The hash symbols (#) in the columns represent the buildings and the pipe symbols (|) in the columns represent the streets. The program must set the gap between buildings as K units by widening the streets. Finally, the program must print the matrix representing the revised city map.
...
li=[input().strip() for i in range(r)]
n=int(input())
for i in li:
    for c in i:
        if c=='|':
