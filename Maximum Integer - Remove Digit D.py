# Maximum Integer - Remove Digit D
# The program must accept an integer N and a digit D as the input. The program must print the maximum possible integer value that can be formed by removing the digit D exactly once in the integer N.
...
while f in s:
    ...
    s[x]='*'
    t = s[:x]+s[x+1:]
    arr.append(int(''.join(''.join(t).replace('*',f))))
print(max(arr))